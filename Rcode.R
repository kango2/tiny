...
g <- b %>% dplyr::filter(qlen>500 | tlen>500) %>% 
  ...
  theme_bw() + 
  scale_x_continuous(breaks = arrange(b, trank) %>% pull(toffset) %>% unique(), minor_breaks = NULL, labels = arrange(b, trank) %>% pull(tchr) %>% unique()) + 
  scale_y_continuous(breaks = arrange(b, qrank) %>% pull(qoffset) %>% unique(), minor_breaks = NULL, labels = arrange(b, qrank) %>% pull(qchr) %>% unique())
dotplot <- ggplotly(g)
dotplot
htmlwidgets::saveWidget(dotplot, "dotplot.html")
